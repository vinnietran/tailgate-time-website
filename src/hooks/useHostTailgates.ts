...
function normalizeTailgate(id: string, data: Record<string, unknown>): TailgateEvent {
  ...
  return {
    id,
    hostUserId: String(data.hostUserId ?? data.hostId ?? data.ownerId ?? ""),
...
export function useHostTailgates(
  hostUserId?: string,
  ...
  useEffect(() => {
    ...
    const rawConfigs: Array<{ key: string; field: string; value?: string | null }> = [
      ...
      { key: "hostEmail", field: "hostEmail", value: hostEmail },
      { key: "hostName", field: "hostName", value: hostName }
    ];
    ...
    const unsubscribers = queryConfigs.map((config) => {
      ...
      return onSnapshot(
        ...
        (snapshot) => {
          ...
          const merged = mergeAndSortTailgates(Array.from(groups.values()));
          setTailgates(merged);
          setLoading(false);
          setError(null);
