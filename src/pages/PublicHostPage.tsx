...
export default function PublicHostPage() {
  ...
  return <HostPageView data={data} />;
}

export function HostPageView({ data, preview = false }: { data: PublicHostPageData; preview?: boolean }) {
  ...
  useEffect(() => {
    if (!preview) trackPaidTailgateImpressions(data?.upcomingTailgates ?? [], "host_page");
  }, [data?.upcomingTailgates, preview]);
  ...
  return (
    <div className="host-public-page">
      {!preview ? <HostPageSeo profile={profile} /> : null}
      {!preview ? <PublicTopNav /> : null}
      <main>
        <section
        ...
        >
          ...
          <div className="host-public-hero-content">
            ...
            <div className="host-public-hero-actions">
              <a className="host-public-events-cta" href="#host-events">Explore events ↓</a>
            <button type="button" className="host-public-share" onClick={preview ? undefined : share} disabled={preview}>{shareLabel}</button>
            </div>
          </div>
        </section>

        <nav className="host-public-section-nav" aria-label="Host page sections">
          <a href="#host-events">Events <span>{upcomingTailgates.length}</span></a>
          {profile.description ? <a href="#host-about">About</a> : null}
          {galleryImages.length ? <a href="#host-gallery">Photos</a> : null}
        </nav>
        <div className="host-public-content">
          <section id="host-events" className="host-public-events">
            ...
            {upcomingTailgates.length ? (
                ...
                {upcomingTailgates.map((event) => {
                  ...
                  return (
                    <Link
                      ...
                      onClick={(click) => { if (preview) { click.preventDefault(); return; } void trackHostPageEvent("host_page_event_click", {
              ...
              <div className="host-public-empty">
                ...
                <p>New dates will appear here. Save this page for your next game day.</p>
              </div>
            )}
          </section>
          {profile.description ? (
            <section id="host-about" className="host-public-about">
          ...
          {galleryImages.length ? (
            <section id="host-gallery" className="host-public-gallery" aria-labelledby="host-public-gallery-title">
              <div className="host-public-section-heading">
                <div>
